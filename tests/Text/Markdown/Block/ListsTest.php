...
class ListsTest extends TestCase
{
	...
	public function testConsumeUnordered(): void
	{
		...
		$this->assertSame(['Red'], $node->children[0]->content);
		$this->assertSame(['Green'], $node->children[1]->content);
	}
	...
	public function testConsumeOrdered(): void
	{
		...
		$this->assertSame(['Bird'], $node->children[0]->content);
	}
	...
	{
		// five or more spaces after the marker keep the
		// surplus indentation as part of the item content
		$line = new Line(['-     Red']);
		$node = $this->block->consume($line);
	...
	public function testConsumeBareMarker(): void
	{
		...
		$this->assertSame([], $node->children[0]->content);
	}
	...
	public function testConsumeLooseList(): void
	{
		...
		// each item content is terminated with a blank line
		$line = new Line(['-   Red', '', '-   Green']);
		$node = $this->block->consume($line);
	...
	{
		// indented content after a blank line is appended
		// to the current item and turns the list loose
		$line = new Line(['-   Red', '', '    more']);
		$node = $this->block->consume($line);

		$this->assertSame('ul', $node->name);
		$this->assertCount(1, $node->children);
		$this->assertSame(['Red', '', 'more', ''], $node->children[0]->content);
	}
	...
	public function testConsumeLazyContinuation(): void
	{
		...
		$this->assertSame('ul', $node->name);
		$this->assertCount(1, $node->children);
		$this->assertSame(['Red', 'lazy text'], $node->children[0]->content);
	}

	public function testConsumeParagraphAfterBlankEndsList(): void
	{
		// a non-indented paragraph after a blank line ends the list
		...
		$this->assertCount(1, $node->children);
		$this->assertSame(['Red'], $node->children[0]->content);
		$this->assertTrue($line->matches('/^after blank/'));
	}
