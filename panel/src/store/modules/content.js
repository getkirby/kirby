    ...
    mutations: {
      ...
      UNLOCK(state, unlock) {
        if (unlock) {
          ...
          state.models[state.current].changes = {};
        }

        state.status.unlock = unlock;
      },
      UPDATE(state, [id, field, value]) {
        ...
        if (original == current) {
          ...
          app.delete(state.models[id].changes, field);
        } else {
          // if they differ, set as unsaved change
          state.models[id].changes[field] = value;
        }
