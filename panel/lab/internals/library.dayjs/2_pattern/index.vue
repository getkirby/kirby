	...
	<k-lab-examples>
		...
		<k-lab-example label="dayjs.pattern() getters" :code="false">
			...
			<k-grid variant="fields">
				<k-column width="1/2">
					<k-label><k-code-token>.parts</k-code-token></k-label>
		...
		<k-lab-example label="dayjs.pattern().at()" :code="false">
			<k-text>
				<p>
					...
					second index to look up a selection instead of a caret.
				</p>
			</k-text>

			<!-- prettier-ignore -->
			<k-code language="javascript">this.$library.dayjs.pattern("{{ pattern }}").at(start, end): object|undefined</k-code>

			<k-grid variant="fields">
				<k-column width="1/2">
					<k-label>Cursor position</k-label>
					<k-input
						type="number"
						min="0"
						:max="pattern.length"
...
export default {
	...
	computed: {
		...
		part() {
			return this.pattern.at(Number(this.position));
		},
		pattern() {
